...
//        return new SuccessResource($response);
     ...
     */
    public function getFiltered(ListRequest $request)
    {
        if($request->has('category') && !empty($request->category)) {
            $categories_array = $this->categoryRepository->getCategoriesArray($request->category);
            $request->merge(['categories' => $categories_array]);
        }
